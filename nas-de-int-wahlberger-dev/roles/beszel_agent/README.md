...
repo has already hit real SELinux/device-passthrough friction twice getting
a *container* that kind of host access right (see `roles/samba/README.md`'s
SELinux section and `roles/immich/README.md`'s hardware-acceleration
section). A native binary just has normal host access without any of that,
and Beszel ships a static binary specifically for this.
...
this host means adding it to `firewall_allowed_ports` in `group_vars`
...
the above): reaching a socket file at all requires *search* permission on
every directory component of its path, not just the right group on the
file itself — the exact same rule the `samba` role's SELinux notes
describe for a different reason. `podman.socket` creates `/run/podman`
itself on activation, and its default mode isn't guaranteed the same
across hosts or Podman versions — found `0700 root:root` here (blocking
`beszel_agent_user` outright, regardless of the socket file's own group),
`0755` on another host in this repo with the exact same role applied. This
role now also forces `/run/podman` to `0755`, group `beszel_agent_podman_group`
— and, since `/run` is tmpfs and gets recreated every boot (undoing a plain
one-off `chmod`), a `tmpfiles.d` rule reapplies it automatically on every
future boot too, before regular services (including `podman.socket`) start.

## SELinux

Same situation and same fix as `roles/samba/README.md` describes: recent
openSUSE Tumbleweed snapshots default to SELinux (targeted policy), and a
binary placed outside the package manager's usual paths starts out
unlabeled — a confined systemd service fails to execute it. This role
labels `beszel_agent_bin_path` as `bin_t` (`community.general.sefcontext` +
`restorecon`), unconditionally — a no-op if SELinux isn't enabled.
