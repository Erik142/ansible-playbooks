...
- Idempotency + deprecation-warning check (CI, and locally with Docker):
  ...
  `ssh_authorized_keys` twice against a disposable container and fails on
  any `changed` the second time or any ansible-core deprecation warning.
  Deliberately excludes roles needing real mounted drives, Podman-in-Docker,
  or real external API credentials — see `molecule/default/converge.yml`.
